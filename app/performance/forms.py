...
class indicatorForm(FlaskForm):
    ...
    indicatorMeasurementFrequency = SelectField('Measurement Frequency',
                                                validators=[],
                                                choices=[],
                                                widget=select2Widget())
    indicatorUOM = SelectField('Unit of measure',
                               choices=[],
                               widget=select2Widget())
    indicatorProcessType = SelectField('Process type',
                                       choices=[],
                                       widget=select2Widget())
    indicatorIndicatorType = SelectField('Indicator type',
                                         choices=[],
                                         widget=select2Widget())
    indicatorGoodPerformance = SelectField('Good performance',
                                           choices=[],
                                           widget=select2Widget())
